...
class LikesControllerTest {
    ...
    void isLikedThisPostTest() throws Exception {
        mock.perform(get("/users/test/likes/posts/1")).andExpect(status().isOk());
    }

    @Test
    @DisplayName("특정 게시물 좋아요 활성화")
    void likesOnTest() throws Exception {
        mock.perform(post("/users/test/likes/posts/1")).andExpect(status().isOk());
    }

    @Test
    @DisplayName("특정 게시물 좋아요 비활성화")
    void likesOffTest() throws Exception {
        mock.perform(delete("/users/test/likes/posts/1")).andExpect(status().isOk());
    }

    @Test
    @DisplayName("특정 게시물을 좋아요한 유저 리스트 조회")
    void likesUsersTest() throws Exception {
        mock.perform(get("/posts/1/likes/users")).andExpect(status().isOk());
    }

    @Test
    @DisplayName("특정 유저의 좋아요한 게시물 리스트 조회")
    void likesPostsTest() throws Exception {
        mock.perform(get("/users/test/likes/posts")).andExpect(status().isOk());
    }
}